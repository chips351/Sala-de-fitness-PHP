...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // 1. Validare date (fără verificare email unic)
        $errors = $user->validateEditProfile($nume, $email, $telefon, $parola, $parolaNoua, $user->getPassword());
        // 2. Verificare email unic doar dacă email-ul s-a schimbat și e valid
        if (empty($errors) && strtolower($email) !== strtolower($user->getEmail())) {
            if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                if (User::emailExists($email)) {
                    $errors[] = "Email-ul '$email' există deja!";
                }
            }
        }
        if (!empty($errors)) {
        ...
        if ($user->save()) {
            ...
            $response['errors'][] = 'save() return: ' . var_export($user->save(), true);
    ...
    <div class="bg-gradient-to-b from-black/50 to-white/50 backdrop-blur-sm shadow-xl rounded-3xl p-10 w-[450px] flex flex-col items-center">
        ...
        <form id="editProfileForm" method="POST" class="w-full flex flex-col items-center" autocomplete="off">
            ...
            <div class="w-full mb-5 relative">
                <label class="block text-left text-gray-700 font-medium mb-1">Parola actuală (pentru modificare parolă)</label>
